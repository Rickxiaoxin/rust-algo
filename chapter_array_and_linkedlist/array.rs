...
fn main() {
    ...
    // 在rust中，指定长度时([i32;5])为数组
    // 由于Rust的数组被设计为在编译期确定长度，因此只能使用常量来指定长度
    // 为了方便实现扩容 extend() 方法，以下将Vec看作数组(Array)，也是Rust一般情况下使用动态数组的类型
